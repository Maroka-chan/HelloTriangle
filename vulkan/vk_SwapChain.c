...
static VkExtent2D chooseSwapExtent(GLFWwindow* window, VkSurfaceCapabilitiesKHR capabilities) {
...
SwapChainDetails createSwapChain(GLFWwindow* window, VkDevice device, VkPhysicalDevice physicalDevice, VkSurfaceKHR surface) {
  ...
  VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(swapChainSupport.formats, swapChainSupport.surfaceFormatCount);
...
void destroySwapChainSupportDetails(SwapChainSupportDetails* details) { // TODO Destroy somewhere. Probably save them in a global list and destroy on Cleanup()??
  if (details->surfaceFormatCount != 0) free(details->formats);
  if (details->presentModeCount != 0) free(details->presentModes);
}
